 ...
 *
 * @package Menu_Item_Custom_Fields_LeftMenu_Checkbox
 * @version 0.2.0
 ...
 * @since 0.1.0
 */
class Menu_Item_Custom_Fields_LeftMenu_Checkbox {
	 ...
	 * @var    array
	 * @access protected
	 * @since  Menu_Item_Custom_Fields_LeftMenu_Checkbox 0.2.0
	 */
	...
	public static function init() {
		...
		self::$fields = array(
			'leftmenu_item_type_field' => __( 'Là thanh ngang', 'leftmenu_item_type_field' ),
		);
	}
		...
		foreach ( self::$fields as $_key => $label ) :
			...
			?>
				<p class="description description-wide <?php echo esc_attr( $class ) ?>">
					<?php printf(
						'<label for="%1$s">%2$s<br /><input type="checkbox" id="%1$s" class="widefat %1$s" name="%3$s" %4$s /></label>',
						esc_attr( $id ),
						esc_html( $label ),
						esc_attr( $name ),
						esc_attr( $value )
...
Menu_Item_Custom_Fields_LeftMenu_Checkbox::init();
